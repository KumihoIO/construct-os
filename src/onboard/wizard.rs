...
const BANNER: &str = r"
    ...
    Memory-native. Kumiho-powered. Multi-agent.
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to Construct — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    // memories, deprecates stale ones, enriches tags/metadata, and creates
    // graph edges between related items.  Recommended to run nightly.
    if is_kumiho_backend {
        ...
        if setup_dream_state {
            ...
            match crate::cron::add_agent_job(
                ...
                None,  // delivery: none
                false, // delete_after_run: recurring
                Some(vec!["kumiho_memory_dream_state".to_string()]),
            ) {
                Ok(job) => println!(
...
/// Result of memory setup — includes both MemoryConfig and optional Kumiho overrides.
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to Construct from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to Construct from Telegram").dim()
                );
                ...
                print_bullet(
                    "Use your @username without '@' (example: yourname), or your numeric Telegram user ID.",
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to Construct from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to Construct from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("Construct reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.construct/state/whatsapp-web/session.db")
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("construct-whatsapp-verify")
                    .interact_text()?;
            ...
            ChannelMenuChoice::Lark | ChannelMenuChoice::Feishu => {
                ...
                println!(
                    ...
                    style(format!("— talk to Construct from {provider_label}")).dim()
                );
                print_bullet(&format!(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("Construct will listen for encrypted DMs on Nostr relays.");
...
async fn scaffold_workspace(
    ...
    // Seed built-in workflow YAMLs (skip-if-exists; no-op when there are none).
...
mod tests {
    ...
    // ── scaffold_workspace: AGENTS.md references Kumiho memory tools
    ...
    fn backend_key_from_choice_maps_supported_backends() {
        ...
        // Out-of-range falls back to the default (kumiho).
