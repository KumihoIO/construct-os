...
impl AuditLogger {
    ...
    ///
    /// If `config.sign_events` is true, requires `CONSTRUCT_AUDIT_SIGNING_KEY` env var
    /// to be set with a hex-encoded 32-byte key. Fails if key is missing or invalid.
    pub fn new(config: AuditConfig, construct_dir: PathBuf) -> Result<Self> {
        // Load and validate signing key if sign_events enabled
        let signing_key = if config.sign_events {
            let key_hex = std::env::var("CONSTRUCT_AUDIT_SIGNING_KEY").map_err(|_| {
                anyhow::anyhow!("sign_events enabled but CONSTRUCT_AUDIT_SIGNING_KEY not set")
            })?;

            let key_bytes = hex::decode(&key_hex)
                .map_err(|_| anyhow::anyhow!("CONSTRUCT_AUDIT_SIGNING_KEY must be hex-encoded"))?;

            if key_bytes.len() != 32 {
                bail!(
                    "CONSTRUCT_AUDIT_SIGNING_KEY must be 32 bytes (64 hex chars), got {}",
                    key_bytes.len()
                );
            }
        ...
        let log_path = construct_dir.join(&config.log_path);

        // Ensure parent directory exists and touch the log file so readers
        // never encounter a "not found" error on a fresh install. Skipped when
        // auditing is disabled so the logger leaves no artifact on disk.
        if config.enabled {
    ...
    ///
    /// Returns up to `limit` events, optionally filtered by `event_type` and/or
    /// time range (`since`). Events are returned in reverse-chronological order
    /// (newest first).
    pub fn read_events(
        &self,
        limit: usize,
        ...
        for line in reader.lines() {
            ...
            if let Some(ref filter_type) = event_type {
                let type_str = serde_json::to_string(&entry.event_type)
                    .unwrap_or_default()
                    .trim_matches('"')
                    .to_string();
    ...
    pub fn log(&self, event: &AuditEvent) -> Result<()> {
        ...
        // Hold the chain mutex across sequence assignment AND the file append.
        // Without this, concurrent writers could interleave bytes in the file
        // or produce duplicate sequence numbers (the chain mutex serializes
        // the in-memory counter, but the file write used to happen outside
        // the lock, so the OS could reorder appends).
        ...
        // Advisory exclusive lock defends against a second daemon instance
        // (e.g., a brief launchctl respawn overlap) writing to the same log.
        // Released on drop.
        file.lock()?;
        writeln!(file, "{}", line)?;
        file.sync_all()?;

        // Advance state only after a successful write so an I/O failure
        // cannot leave a sequence gap.
    ...
    pub fn log_auth_success(&self, channel: &str, detail: &str) -> Result<()> {
        let event = AuditEvent::new(AuditEventType::AuthSuccess)
            .with_actor(channel.to_string(), None, None)
            .with_action(detail.to_string(), "none".to_string(), true, true);
        self.log(&event)
    }

    /// Log an authentication failure event.
    pub fn log_auth_failure(&self, channel: &str, detail: &str) -> Result<()> {
        let event = AuditEvent::new(AuditEventType::AuthFailure)
            .with_actor(channel.to_string(), None, None)
            .with_action(detail.to_string(), "high".to_string(), false, false);
        self.log(&event)
    }

    /// Log a configuration change event.
    pub fn log_config_change(&self, channel: &str, detail: &str) -> Result<()> {
        let event = AuditEvent::new(AuditEventType::ConfigChange)
            .with_actor(channel.to_string(), None, None)
            .with_action(detail.to_string(), "medium".to_string(), true, true);
        self.log(&event)
    }

    /// Log a security event (e.g. WebSocket connection, pairing).
    pub fn log_security_event(&self, channel: &str, detail: &str) -> Result<()> {
        let event = AuditEvent::new(AuditEventType::SecurityEvent)
            .with_actor(channel.to_string(), None, None)
            .with_action(detail.to_string(), "low".to_string(), true, true);
        self.log(&event)
    }

    /// Rotate log if it exceeds max size. Caller must hold the chain state so
    /// the sequence/prev-hash reset is atomic with the rename.
    fn rotate_if_needed(&self, state: &mut ChainState) -> Result<()> {
    ...
    /// Rotate the log file and reset the in-memory Merkle chain to genesis.
    ///
    /// The new current log starts a fresh chain (sequence 0, genesis prev_hash)
    /// so `verify_chain` on the rotated file succeeds immediately. The archived
    /// `.N.log` backups remain individually verifiable: each is its own chain.
    fn rotate(&self, state: &mut ChainState) -> Result<()> {
        ...
        state.prev_hash = GENESIS_PREV_HASH.to_string();
        state.sequence = 0;
        Ok(())
    }
}
...
/// - If a record has a `signature` field and `CONSTRUCT_AUDIT_SIGNING_KEY` is available,
...
pub fn verify_chain(log_path: &Path) -> Result<u64> {
    let file = match std::fs::File::open(log_path) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            // No log file yet — chain is trivially valid with 0 entries.
            return Ok(0);
        }
        Err(e) => return Err(e.into()),
    };
    ...
    let signing_key = std::env::var("CONSTRUCT_AUDIT_SIGNING_KEY")
        .ok()
...
mod tests {
    ...
    /// Mutex to serialize tests that read/write CONSTRUCT_AUDIT_SIGNING_KEY env var.
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@construct_user".to_string()));
    }
    ...
    #[test]
    fn audit_rotation_resets_chain_so_verify_passes() -> Result<()> {
        // Regression: before the fix, `rotate()` renamed the active log but
        // left the in-memory sequence / prev_hash intact. The next append into
        // the fresh (empty) log started at sequence=N with a stale prev_hash,
        // and `verify_chain` failed on the first entry with a sequence gap.
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            max_size_mb: 10,
        ...
        // Write a few pre-rotation events so state.sequence advances.
        for _ in 0..3 {
            logger.log(&AuditEvent::new(AuditEventType::SecurityEvent))?;
        }

        // Force a rotation by toggling the size limit via a fresh logger
        // pointing at the same log. (The production path triggers rotation
        // by file size; here we simulate that via a zero-MB config.)
        let force_rotate_cfg = AuditConfig {
            enabled: true,
            max_size_mb: 0,
            ..Default::default()
        };
        let rotating_logger = AuditLogger::new(force_rotate_cfg, tmp.path().to_path_buf())?;
        rotating_logger.log(&AuditEvent::new(AuditEventType::SecurityEvent))?;

        let log_path = tmp.path().join("audit.log");
        let backup = format!("{}.1.log", log_path.display());
    ...
    #[test]
    fn concurrent_writes_produce_verifiable_chain() -> Result<()> {
        // Regression: the file append used to happen outside the chain mutex,
        // so two threads could interleave bytes into the same line or race on
        // sequence assignment. With the fix, N concurrent writers produce a
        // chain that verifies cleanly with contiguous sequence numbers.
        use std::sync::Arc;
        use std::thread;
        ...
        let logger = Arc::new(AuditLogger::new(config, tmp.path().to_path_buf())?);
    ...
    fn signature_present_when_sign_events_enabled() -> Result<()> {
        ...
        let old_key = std::env::var("CONSTRUCT_AUDIT_SIGNING_KEY").ok();
        defer! {
            if let Some(key) = old_key {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::set_var("CONSTRUCT_AUDIT_SIGNING_KEY", key) };
            } else {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::remove_var("CONSTRUCT_AUDIT_SIGNING_KEY") };
            }
        }
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("CONSTRUCT_AUDIT_SIGNING_KEY", &test_key) };
    ...
    fn constructor_fails_if_sign_events_but_no_key() -> Result<()> {
        ...
        if let Err(e) = result {
            ...
            assert!(
                err_msg.contains("CONSTRUCT_AUDIT_SIGNING_KEY not set"),
                "error: {}",
                err_msg
            );
        }
    ...
    fn constructor_fails_if_signing_key_invalid_hex() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("CONSTRUCT_AUDIT_SIGNING_KEY", "not-valid-hex") };
    ...
    fn constructor_fails_if_signing_key_wrong_length() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("CONSTRUCT_AUDIT_SIGNING_KEY", &short_key) };
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            sign_events: true,
