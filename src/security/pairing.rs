...
impl PairingGuard {
    ...
    fn try_pair_blocking(&self, code: &str, client_id: &str) -> Result<Option<String>, u64> {
                ...
                if let Some(until) = state.lockout_until {
                    if now < until {
                        ...
                        tracing::warn!(
                            ...
                            "try_pair_blocking: client locked out"
                        );
...

        {
            ...
            if pairing_code.is_none() {
                tracing::debug!(
                    client_id = %client_id,
                    "try_pair_blocking: no active pairing code (expired or unconfigured)"
                );
            }
            if let Some(ref expected) = *pairing_code {
                if constant_time_eq(code.trim(), expected.trim()) {
                    ...
                    *pairing_code = None;
                    ...
                    tracing::info!(
                        client_id = %client_id,
                        "try_pair_blocking: code matched, token issued"
                    );
        ...
        // Increment failed attempts for this client
        {
            ...
            let entry = map.entry(client_id.clone()).or_insert(FailedAttemptState {
                count: 0,
                lockout_until: None,
                last_attempt: now,
            });
            ...
            let will_lockout = entry.count >= MAX_PAIR_ATTEMPTS;
            if will_lockout {
            ...
            tracing::warn!(
                ...
                will_lockout,
                "try_pair_blocking: code mismatch, failed attempt recorded"
            );
        }
    ...
    pub fn generate_new_pairing_code(&self) -> Option<String> {
        if !self.require_pairing {
            tracing::debug!("generate_new_pairing_code: pairing disabled, returning None");
            return None;
        }
        let new_code = generate_code();
        *self.pairing_code.lock() = Some(new_code.clone());
        let prefix: String = new_code.chars().take(2).collect();
        tracing::info!(code_prefix = %prefix, "generate_new_pairing_code: new code issued");
        Some(new_code)
    }
