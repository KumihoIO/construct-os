...
impl SecretStore {
    ...
    fn load_or_create_key(&self) -> Result<Vec<u8>> {
        if self.key_path.exists() {
            ...
            // Unix: create-and-chmod atomically via O_CREAT|O_EXCL plus mode
            // 0o600 on the syscall so no window exists where the file is
            // readable by same-group attackers. Falls back to fs::write on
            // non-Unix, where ACLs are applied below.
            #[cfg(unix)]
            {
                use std::io::Write;
