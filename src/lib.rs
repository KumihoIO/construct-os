...
pub enum WorkflowCommands {
    /// List built-in workflows bundled with this binary.
    List,
    /// Seed (or refresh) built-in workflows into the workspace.
    #[command(long_about = "\
Copy the workflow YAMLs bundled with this binary into the active \
workspace at `operator_mcp/workflow/builtins/`. Existing files are left \
alone unless --force is passed.")]
    Sync {
        /// Overwrite workflow files even when they already exist on disk.
        #[arg(long)]
        force: bool,
    },
}
