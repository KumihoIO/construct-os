...
pub async fn run(config: Config) -> Result<()> {
    ...
    // ── Sync workflow cron triggers from builtin and user workflows.
    match sync_workflow_cron_triggers_at_startup(&config).await {
        Ok(count) => {
            if count > 0 {
                tracing::info!("Synced {count} workflow cron trigger(s) at startup");
            }
        }
        Err(e) => tracing::warn!("Failed to sync workflow cron triggers: {e}"),
    }
...
async fn execute_job_with_retry(
    ...
    for attempt in 0..=retries {
        let (success, output) = match job.job_type {
            ...
            JobType::Workflow => Box::pin(run_workflow_job(config, job)).await,
        };
        last_output = output;
...
/// Execute a workflow cron job by triggering the workflow via the gateway API.
async fn run_workflow_job(config: &Config, job: &CronJob) -> (bool, String) {
    let workflow_name = &job.command; // workflow name stored in command field

    // Determine gateway URL
    let port = config.gateway.port;
    let host = &config.gateway.host;
    let url = format!("http://{host}:{port}/api/workflows/run/{workflow_name}");

    tracing::info!("Triggering workflow run via cron: {workflow_name}");

    let client = reqwest::Client::new();
    let body = serde_json::json!({
        "inputs": {},
        "cwd": config.workspace_dir.display().to_string(),
    });
        ...
        .await
    {
        Ok(resp) => {
            ...
            if status.is_success() {
                tracing::info!("Workflow {workflow_name} triggered successfully: {text}");
                (true, format!("Workflow {workflow_name} triggered: {text}"))
            } else {
                tracing::warn!("Workflow {workflow_name} trigger failed ({status}): {text}");
                (false, format!("HTTP {status}: {text}"))
            }
        }
        Err(e) => {
            tracing::error!("Failed to trigger workflow {workflow_name}: {e}");
            (false, format!("Request failed: {e}"))
        }
    }
}
...
/// Scan workflow definitions for cron triggers and sync to scheduler.
async fn sync_workflow_cron_triggers_at_startup(config: &Config) -> Result<usize> {
    let mut all_triggers: Vec<(String, String, Option<String>)> = Vec::new();

    // 1. Scan builtin workflows directory
        ...
        for entry in entries.flatten() {
            ...
            if path.extension().is_some_and(|e| e == "yaml" || e == "yml") {
                if let Ok(content) = std::fs::read_to_string(&path) {
                    let name = extract_workflow_name(&content).unwrap_or_else(|| {
                        path.file_stem()
                            .unwrap_or_default()
                            .to_string_lossy()
                            .to_string()
                    });
                    for (expr, tz) in extract_cron_from_yaml(&content) {
                        all_triggers.push((name.clone(), expr, tz));
                    }
                }
            }
        }
    ...
    // 2. Try to fetch Kumiho workflows via gateway API (may fail if gateway isn't ready yet)
    let port = config.gateway.port;
    let host = &config.gateway.host;
    let url = format!("http://{host}:{port}/api/workflows");
        ...
        .await
    {
        if let Ok(body) = resp.json::<serde_json::Value>().await {
            if let Some(workflows) = body.get("workflows").and_then(|v| v.as_array()) {
                for wf in workflows {
                    let name = wf
                        .get("item_name")
                        .or(wf.get("name"))
                    ...
                    let definition = wf
                        ...
                        .unwrap_or_default();
                    if !name.is_empty() {
                        for (expr, tz) in extract_cron_from_yaml(definition) {
                            all_triggers.push((name.clone(), expr, tz));
                        }
                    }
                }
    ...
    let count = all_triggers.len();
    if !all_triggers.is_empty() {
        crate::cron::store::sync_workflow_cron_jobs(config, &all_triggers)?;
    }
...
fn extract_cron_from_yaml(content: &str) -> Vec<(String, Option<String>)> {
    ...
    for line in content.lines() {
        ...
        if in_triggers && !line.starts_with(' ') && !trimmed.is_empty() && !trimmed.starts_with('#')
        {
            in_triggers = false;
        }
        if !in_triggers {
        ...
        {
            let expr = expr.trim().trim_matches('"').trim_matches('\'').to_string();
            if !expr.is_empty() {
                results.push((expr, None));
            }
        }
    }
    results
