...
/// Sanitize a string into a URL/ID-safe slug.
...
/// Reconcile cron jobs sourced from workflow YAML triggers.
///
/// Each entry is `(workflow_name, cron_expression, optional_tz)`.
/// Jobs use `source = 'workflow'` and deterministic IDs so they survive
/// across restarts without duplication.
pub fn sync_workflow_cron_jobs(
    config: &Config,
    workflows: &[(String, String, Option<String>)], // (name, cron_expr, tz)
) -> Result<()> {
    if workflows.is_empty() {
        // If no workflow triggers are defined, clean up any stale workflow jobs.
        with_connection(config, |conn| {
            let deleted = conn
                .execute("DELETE FROM cron_jobs WHERE source = 'workflow'", [])
                .context("Failed to remove stale workflow cron jobs")?;
            if deleted > 0 {
                tracing::info!(
                    count = deleted,
                    "Removed workflow cron jobs no longer present"
                );
            }
            Ok(())
        })?;
    ...
    with_connection(config, |conn| {
        // Build the set of expected job IDs.
        let expected_ids: Vec<String> = workflows
            .iter()
            .enumerate()
            .map(|(idx, (name, _expr, _tz))| format!("__wf_cron_{}_{}", slug(name), idx))
            .collect();

        // Remove stale workflow jobs no longer in the set.
        {
            let mut stmt = conn.prepare("SELECT id FROM cron_jobs WHERE source = 'workflow'")?;
            let db_ids: Vec<String> = stmt
                .query_map([], |row| row.get(0))?
                .filter_map(|r| r.ok())
                .collect();

            for old_id in &db_ids {
                if !expected_ids.contains(old_id) {
                    conn.execute(
                        "DELETE FROM cron_jobs WHERE id = ?1 AND source = 'workflow'",
                        params![old_id],
                    )
                    .with_context(|| {
                        format!("Failed to remove stale workflow cron job '{old_id}'")
                    })?;
                    tracing::info!(
                        job_id = %old_id,
                        "Removed stale workflow cron job"
                    );
                }
            }
        }

        // Upsert each workflow cron trigger.
        for (idx, (name, expr, tz)) in workflows.iter().enumerate() {
            let job_id = format!("__wf_cron_{}_{}", slug(name), idx);
            ...
            let next_run = match next_run_for_schedule(&schedule, now) {
                ...
                Err(e) => {
                    tracing::warn!(
                        workflow = %name,
                        expr = %expr,
                        "Invalid cron expr for workflow trigger, skipping: {e}"
                    );
                    continue;
                }
            };
            ...
            let exists: bool = conn
                .prepare("SELECT COUNT(*) FROM cron_jobs WHERE id = ?1")?
                .query_row(params![job_id], |row| row.get::<_, i64>(0))
                .map(|c| c > 0)
                .unwrap_or(false);

            if exists {
                // Update schedule if changed, preserve runtime state.
                let current_schedule_raw: Option<String> = conn
                    .prepare("SELECT schedule FROM cron_jobs WHERE id = ?1")?
                ...
                if schedule_changed {
                    conn.execute(
                        "UPDATE cron_jobs
                         SET expression = ?1, schedule = ?2, next_run = ?3,
                             command = ?4, name = ?5, source = 'workflow'
                         WHERE id = ?6 AND source = 'workflow'",
                        params![
                            expr,
                            schedule_json,
                            next_run.to_rfc3339(),
                    ...
                    .with_context(|| format!("Failed to update workflow cron job '{job_id}'"))?;
                } else {
                    conn.execute(
                        "UPDATE cron_jobs
                         SET command = ?1, name = ?2, source = 'workflow'
                         WHERE id = ?3 AND source = 'workflow'",
                        params![name, format!("Workflow: {name}"), job_id],
                    )
                    .with_context(|| format!("Failed to update workflow cron job '{job_id}'"))?;
                }
                ...
                tracing::debug!(
                    job_id = %job_id,
                    expr = %expr,
                    "Updated workflow cron job"
                );
            } else {
                // Insert new workflow cron job.
                conn.execute(
                    "INSERT INTO cron_jobs (
                        id, expression, command, schedule, job_type, name,
                        enabled, source, created_at, next_run,
                        session_target, delete_after_run
                     ) VALUES (?1, ?2, ?3, ?4, 'workflow', ?5, 1, 'workflow', ?6, ?7, 'isolated', 0)",
                    params![
                        job_id,
                        expr,
                        name,
                ...
                .with_context(|| {
                    format!("Failed to insert workflow cron job '{job_id}' for {name}")
                })?;
...
/// Remove all cron jobs sourced from a single workflow.
///
/// Deletes jobs whose `source = 'workflow'` and whose `id` starts with the
/// deterministic prefix `__wf_cron_{slug}_`.
pub fn remove_workflow_cron_jobs(config: &Config, workflow_name: &str) -> Result<()> {
    let prefix = format!("__wf_cron_{}_%", slug(workflow_name));
    with_connection(config, |conn| {
        let deleted = conn
            .execute(
                "DELETE FROM cron_jobs WHERE source = 'workflow' AND id LIKE ?1",
                params![prefix],
            )
            .with_context(|| {
                format!("Failed to remove workflow cron jobs for '{workflow_name}'")
            })?;
        if deleted > 0 {
            tracing::info!(
...
fn validate_decl(decl: &crate::config::schema::CronJobDecl) -> Result<()> {
    ...
    match decl.job_type.to_lowercase().as_str() {
        ...
        "workflow" => {
            // Workflow jobs use the command field as the workflow name;
            // validation of the actual workflow definition happens at trigger time.
            if decl
                .command
                .as_deref()
                .map_or(true, |c| c.trim().is_empty())
            {
                anyhow::bail!(
                    "Declarative cron job '{}': workflow job requires a non-empty 'command' (workflow name)",
                    decl.id
                );
            }
        }
        other => {
            anyhow::bail!(
                "Declarative cron job '{}': invalid job_type '{}', expected 'shell', 'agent', or 'workflow'",
                decl.id,
                other
            );
        }
