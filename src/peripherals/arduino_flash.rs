...
/// Reject port values that begin with `-` (would be parsed by arduino-cli as
/// a misplaced option) or contain shell metacharacters. Expected shapes are
/// serial paths like `/dev/cu.usbmodem14301` or `COM3`.
fn validate_serial_port_arg(port: &str) -> Result<()> {
    if port.is_empty() {
