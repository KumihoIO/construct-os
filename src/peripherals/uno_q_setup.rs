...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    // Defend against argv injection: ssh/scp parse any argument beginning
    // with `-` as an option, so a host like `-oProxyCommand=sh;id` would
    // execute arbitrary local commands. Reject obviously-hostile forms
    // before they reach Command::args.
    validate_ssh_host_arg(host)?;
...
/// Reject hosts that would be parsed as ssh/scp options or contain shell
/// metacharacters. Keeps it tight: alphanumerics plus `.`, `-`, `_`, `@`, `:`
/// (the last for IPv6 literals inside brackets) — any other byte is rejected.
fn validate_ssh_host_arg(host: &str) -> Result<()> {
    if host.is_empty() {
