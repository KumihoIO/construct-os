//! Flash Construct Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Construct Nucleo firmware. Builds from firmware/nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (construct's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from construct repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Construct Nucleo firmware...");
    ...
    println!("Construct Nucleo firmware flashed successfully.");
