...
//! sketch code and calls this tool. Construct compiles and uploads it — no
...
impl Tool for ArduinoUploadTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        if let Err(e) = validate_serial_port_arg(&self.port) {
            return Ok(ToolResult {
                success: false,
                output: String::new(),
                error: Some(format!("invalid configured port: {e}")),
            });
        }
        ...
        let sketch_name = "construct_sketch";
        let temp_dir = std::env::temp_dir().join(format!("construct_{}", uuid::Uuid::new_v4()));
...
/// Reject port values that begin with `-` or contain shell metacharacters.
/// Expected shapes: serial paths like `/dev/cu.usbmodem14301` or `COM3`.
fn validate_serial_port_arg(port: &str) -> anyhow::Result<()> {
    if port.is_empty() {
