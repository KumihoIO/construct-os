...
/// Build the list of built-in hardware tools for the given device registry.
///
/// Returns an alphabetized `Vec<Box<dyn Tool>>` ready to hand off to the agent
/// loop. The built-in set depends on compiled features and on which adapters
/// (Pico UF2 flasher, Aardvark) are currently available.
async fn build_builtin_hardware_tools(
    devices: std::sync::Arc<tokio::sync::RwLock<DeviceRegistry>>,
) -> Vec<Box<dyn crate::tools::Tool>> {
    let mut tools: std::collections::HashMap<String, Box<dyn crate::tools::Tool>> =
        std::collections::HashMap::new();

    for tool in self::gpio::gpio_tools(devices.clone()) {
    ...
    #[cfg(feature = "hardware")]
    {
        let tool: Box<dyn crate::tools::Tool> =
            Box::new(self::pico_flash::PicoFlashTool::new(devices.clone()));
        let name = tool.name().to_string();
        println!("[registry] loaded built-in: {name}");
        tools.insert(name, tool);

        for tool in self::pico_code::device_code_tools(devices.clone()) {
            let name = tool.name().to_string();
            println!("[registry] loaded built-in: {name}");
            tools.insert(name, tool);
        }
        ...
        if has_aardvark {
            for tool in self::aardvark_tools::aardvark_tools(devices.clone()) {
                let name = tool.name().to_string();
                println!("[registry] loaded built-in: {name}");
                tools.insert(name, tool);
            }
...

    {
        ...
        for alias in aliases {
            if let Some(device) = reg.get_device(alias) {
                ...
                println!("[registry] {alias} ready → {port}");
            }
        }
    }

    let mut pairs: Vec<(String, Box<dyn crate::tools::Tool>)> = tools.into_iter().collect();
...
pub struct HardwareBootResult {
    ...
    /// Content of `~/.construct/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.construct/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.construct/hardware/HARDWARE.md`
/// 2. `~/.construct/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.construct/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".construct").join("hardware"), aliases)
}
...
///
/// Without the feature: loads plugin tools from `~/.construct/tools/` only,
...
pub async fn boot(
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        tracing::info!("Say \"flash my pico\" to install Construct firmware automatically");
    }
    ...
    let mut tools = build_builtin_hardware_tools(devices.clone()).await;
    let device_summary = {
        let reg = devices.read().await;
        reg.prompt_summary()
    };
...
/// Handle `construct hardware` subcommands.
...
fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: construct hardware info --chip {}", chip);
        println!();
...
mod tests {
    ...
    fn hardware_context_contains_device_exec_rule() {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|d| d.home_dir().to_path_buf()) {
            let hw_md = home.join(".construct").join("hardware").join("HARDWARE.md");
            if hw_md.exists() {
