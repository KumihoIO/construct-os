...
/// Timeout for init/list operations during MCP handshake.
/// Tool calls do NOT use this — they rely on the outer per-tool timeout
/// in mcp_client::call_tool (default 180s, max 600s).
...
impl McpTransportConn for StdioTransport {
    async fn send_and_recv(&mut self, request: &JsonRpcRequest) -> Result<JsonRpcResponse> {
        ...
        // No internal deadline — the caller (call_tool or connect) wraps this
        // in its own timeout.  This avoids racing with the Python handler's
        // poll timeout and eliminates orphaned responses that cause cross-wiring.
        loop {
            ...
            // JSON-RPC 2.0 requires response id to match request id.
            // Stale responses from timed-out requests may linger in the pipe —
            // discard them and keep waiting for ours.
            if resp.id != request.id {
                tracing::warn!(
                    "MCP stdio: discarding response with mismatched id \
                     (got {:?}, expected {:?}) — likely stale from a timed-out request",
                    resp.id,
                    request.id
                );
                continue;
