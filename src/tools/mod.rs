...
#[allow(unused_imports)]
pub use progress::{
    NoopProgressSink, ProgressHandle, ProgressSink, ProgressToken, SharedProgressSink, noop_sink,
};
...
pub fn all_tools_with_runtime(
    ...
    let mut tool_arcs: Vec<Arc<dyn Tool>> = vec![
        ...
        Arc::new(CanvasTool::new(
            canvas_store.unwrap_or_else(crate::tools::canvas::global_store),
        )),
    ];

    // discord_search tool was backed by the SQLite memory backend which has
    // been removed; the tool is unavailable until reintroduced on top of Kumiho MCP.
