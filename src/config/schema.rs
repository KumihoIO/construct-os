...
pub struct Config {
    ...
    ///
    /// Automatically injects the Kumiho MCP server and session-bootstrap prompt
    /// into every non-internal agent.  Disable only for testing or air-gapped
    /// deployments where Kumiho is not installed.
    #[serde(default)]
    ...
    /// every non-internal agent, enabling plan-delegate-monitor-synthesize
    /// workflows.
    #[serde(default)]
...
pub struct KumihoConfig {
    ...
    /// Kumiho project for user memories, sessions, and compactions.
    #[serde(default = "default_kumiho_memory_project")]
...
fn default_kumiho_api_url() -> String {
    "https://api.kumiho.cloud".to_string()
}

fn default_kumiho_memory_project() -> String {
    "CognitiveMemory".to_string()
}
...
/// Operator multi-agent orchestration (`[operator]` section).
///
/// Controls automatic injection of the Operator MCP server and system prompt
/// into every non-internal agent.  The operator layer enables the lead agent
/// to spawn, monitor, and synthesize results from worker sub-agents.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct OperatorConfig {
    /// Enable Operator injection for non-internal agents. Default: `true`.
    #[serde(default = "default_true")]
    pub enabled: bool,
    ...
    /// Override max tool-call iterations when the operator is active.
    ///
    /// Operator tasks (research → plan → spawn → monitor) are inherently
    /// multi-step and typically require more iterations than normal agent
    /// turns.  When non-zero this value replaces
    /// `agent.max_tool_iterations` for operator-enabled sessions.
    /// Default: 80.
...
/// ClawHub skill marketplace integration (`[clawhub]` section).
///
/// Enables browsing, searching, and installing skills from the public
/// ClawHub registry (clawhub.ai).  An API token is only required for
/// publishing; anonymous browsing and installing is free.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct ClawHubConfig {
    /// Enable ClawHub integration. Default: `true`.
    #[serde(default = "default_true")]
    pub enabled: bool,

    /// ClawHub API token (`clh_…`) for publishing skills.
    /// Optional — browsing and installing work without a token.
    #[serde(default)]
    pub api_token: Option<String>,

    /// Base URL for the ClawHub API. Default: `"https://clawhub.ai"`.
    #[serde(default = "default_clawhub_api_url")]
    pub api_url: String,
}

fn default_clawhub_api_url() -> String {
...
impl Default for ClawHubConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            api_token: None,
            api_url: default_clawhub_api_url(),
        }
    }
}
...
fn get_default_pricing() -> std::collections::HashMap<String, ModelPricing> {
    ...
    // OpenAI Codex (ChatGPT Responses API — GPT-5 family)
    prices.insert(
...
pub struct MemoryConfig {
    /// `"kumiho"` (persistent via Kumiho MCP) or `"none"` (no-op, in-session only).
    ///
    ...
    /// Prune archived conversation rows older than this many days.
