...
pub struct SlackChannel {
    ...
    /// Process-global registry for pending workflow approvals. When a reply
    /// lands in an approval thread the listener claims the pending approval
    /// and calls the resume endpoint instead of forwarding to the agent.
...
impl SlackChannel {
    ...
    /// Attach the process-global approval registry so approval keyword
    /// matches can be intercepted from Slack messages and routed to the
    /// gateway's resume endpoint. See Discord's wiring in `channels::mod`.
    ...
    /// Check whether an incoming Slack message is a reply in an approval thread
    /// and, if so, atomically claim the pending approval, POST to the gateway
    /// resume endpoint, and post a confirmation reply in-thread. Returns true
    /// when the message was intercepted — the caller should skip forwarding it
    /// to the agent pipeline in that case.
    fn try_intercept_approval(
        &self,
        channel_id: &str,
        ...
        let Some(ref registry) = self.approval_registry else {
            return false;
        };
        // Approvals must be a reply inside the thread that was created by the
        // approval prompt — top-level channel messages never match.
        let Some(thread_ts) = thread_ts else {
            return false;
        };
        let Some((run_id, is_approve, feedback)) =
            registry.match_slack_keyword(channel_id, Some(thread_ts), clean_text)
        else {
            return false;
        };
                ...
                .await
            {
                ...
                Ok(resp) => {
                    tracing::warn!(
                        run_id = %run_id,
                        status = %resp.status(),
                        "Slack: workflow approval endpoint returned error"
                    );
                }
                Err(e) => {
                    tracing::warn!(
                        run_id = %run_id,
                        error = %e,
                        "Slack: failed to call workflow approval endpoint"
                    );
                }
            }
    ...
    async fn listen_socket_mode(
        ...
        loop {
            ...
            while let Some(frame) = read.next().await {
                ...
                // Human approval keyword intercept — check before forwarding
                // to the agent pipeline. Only matches when the incoming message
                // is a reply in the thread captured for the pending approval.
                let inbound_thread_ts = Self::inbound_thread_ts_genuine_only(event);
            ...
            for channel_id in target_channels {
                ...
                if let Some(messages) = data.get("messages").and_then(|m| m.as_array()) {
                    ...
                    for msg in messages.iter().rev() {
                        ...
                        let inbound_thread_ts = Self::inbound_thread_ts_genuine_only(msg);
                        if self.try_intercept_approval(
                            &channel_id,
                            inbound_thread_ts.as_deref(),
                            &normalized_text,
