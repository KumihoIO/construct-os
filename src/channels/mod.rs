...
struct ChannelRuntimeContext {
    ...
    /// MCP registry for direct tool calls (e.g. kumiho memory engage).
...
/// or an empty string if the call fails or returns no results.
///
/// MCP tools return `{"content": [{"type": "text", "text": "<json>"}]}`.
/// The inner `text` field contains the actual kumiho response JSON with
/// `context`, `results`, and `source_krefs` fields.
/// Maximum characters of kumiho engage context to inject into the system

async fn kumiho_engage_for_channel(
    ...
    {
        Ok(raw) => {
            // MCP result is JSON: {"content": [{"type": "text", "text": "..."}]}
            // First unwrap the MCP content envelope to get the inner text.
            let inner_text = extract_mcp_text(&raw);
            tracing::debug!(
                raw_len = raw.len(),
                inner_len = inner_text.len(),
                "Kumiho engage raw response"
            );

            if inner_text.is_empty() {
                tracing::info!("Kumiho engage returned empty response");
                return String::new();
            }

            // The inner text is the kumiho response — could be JSON or plain text.
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&inner_text) {
                // Try `context` field first (formatted summary from engage).
                if let Some(ctx_text) = parsed.get("context").and_then(|v| v.as_str()) {
                    if !ctx_text.is_empty() {
                        tracing::info!(
                            context_chars = ctx_text.len(),
                            "Kumiho engage returned context"
                        );
                        return format!("[Memory context (Kumiho)]\n{ctx_text}");
                    }
                }
                // Fall back to `results` array.
                if let Some(results) = parsed.get("results").and_then(|v| v.as_array()) {
                    if !results.is_empty() {
                        let mut buf = String::from("[Memory context (Kumiho)]\n");
                        for r in results.iter().take(10) {
                            if let Some(content) = r.get("content").and_then(|v| v.as_str()) {
                                let title =
                                    r.get("title").and_then(|v| v.as_str()).unwrap_or("memory");
                                buf.push_str(&format!("- {title}: {content}\n"));
                            }
                        }
                        if buf.len() > "[Memory context (Kumiho)]\n".len() {
                            tracing::info!(
                                results_count = results.len(),
                                "Kumiho engage returned results"
                            );
                            return buf;
                        }
                    }
            ...
            // If not valid JSON, treat the whole inner text as context.
            if !inner_text.is_empty() {
                tracing::info!(
                    text_len = inner_text.len(),
                    "Kumiho engage returned plain text"
                );
                return format!("[Memory context (Kumiho)]\n{inner_text}");
            }
        ...
        Err(e) => {
            tracing::warn!("Kumiho engage failed: {e:#}");
            String::new()
        }
    }
}
...
///
/// MCP results are shaped as:
/// ```json
/// {"content": [{"type": "text", "text": "...actual content..."}]}
/// ```
/// This function returns the inner `text` string, or falls back to
/// treating the raw input as direct content.
fn extract_mcp_text(raw: &str) -> String {
    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(raw) {
        // Standard MCP content array
        if let Some(content_arr) = parsed.get("content").and_then(|v| v.as_array()) {
        ...
        // Maybe it's already a direct response (no content wrapper)
        if parsed.get("context").and_then(|v| v.as_str()).is_some() {
...
async fn process_channel_message(
    ...
    // ── Memory recall (Kumiho MCP or legacy) ───────────────────────
    // When an MCP registry with kumiho-memory is available, call
    // kumiho_memory_engage directly before the LLM call.  The result
    // is injected into the system prompt so the model has context
    // without needing to spend a tool-call turn on recall.
    let mem_recall_start = Instant::now();
    let memory_context = if let Some(ref registry) = ctx.mcp_registry {
        kumiho_engage_for_channel(
            registry,
            &msg.content,
        ...
        // Fallback to legacy dual-scope recall for non-MCP setups.
        let is_group_chat =
    ...
    tracing::info!(
        mem_recall_ms,
        kumiho = ctx.mcp_registry.is_some(),
    ...
    let llm_success = matches!(&llm_result, LlmExecutionResult::Completed(Ok(Ok(_))));
    let reaction_done_emoji = if llm_success {
        "\u{2705}" // ✅
    } else {
        "\u{26A0}\u{FE0F}" // ⚠️
    };
    ...
    // Audit log the completed message processing
    if let Some(ref logger) = ctx.audit_logger {
        let _ = logger.log_command(
            &msg.channel,
            &truncate_with_ellipsis(&msg.content, 120),
            "low",
            true,
            true,
            llm_success,
...
fn collect_configured_channels(
    ...
    if config.channels_config.discord_history.is_some() {
        tracing::warn!(
            "discord_history: persistent history storage has been removed \
             (SQLite backend deleted; channel is disabled). \
             Use Kumiho MCP for persistent memory."
        );
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    // Create audit logger for channel message tracking (shares log file with gateway).
    let audit_logger: Option<Arc<crate::security::audit::AuditLogger>> =
        if config.security.audit.enabled {
            match crate::security::audit::AuditLogger::new(
                ...
                Err(e) => {
                    tracing::warn!("Channel audit logger disabled: {e}");
                    None
                }
            }
        } else {
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        max_tool_iterations: crate::agent::loop_::effective_max_tool_iterations(&config),
