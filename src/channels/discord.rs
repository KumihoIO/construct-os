...
pub struct DiscordChannel {
    ...
    /// Registry of pending human approval requests — when set, enables keyword-based approval
    /// intercept in `listen()`.
...
impl DiscordChannel {
    ...
    /// Configure the approval registry and gateway port for keyword-based approval intercept.
        ...
        loop {
            tokio::select! {
                ...
                msg = read.next() => {
                    ...
                    // ── Human approval keyword intercept ─────────────────────
                    // Check BEFORE forwarding to the agent pipeline.
                    // We check against `clean_content` (mention-stripped, trimmed text)
                    // since approval keywords are standalone text commands.
                    //
                    // Scoping: the incoming message must be either in the thread
                    // we created for this approval, or a Discord reply to the
                    // original approval prompt. The registry enforces that —
                    // here we just extract the relevant IDs. Thread messages
                    // arrive with `channel_id` set to the thread's ID (threads
                    // are themselves channels in Discord's model).
                    let channel_id_for_check = d
                        .get("channel_id")
                        .and_then(|c| c.as_str())
                        .unwrap_or("");
                    ...
                    if let Some(ref registry) = self.approval_registry {
                        if let Some((run_id, is_approve, feedback)) = registry.match_discord_keyword(
                            channel_id_for_check,
                            Some(channel_id_for_check),
                            reply_to_message_id,
                            &clean_content,
                        )
                        {
                            ...
                            if let Some(pending) = registry.try_claim(&run_id) {
                                ...
                                let author_username = d
                                    ...
                                    .unwrap_or("unknown")
                                    .to_string();
                                let reply_channel = channel_id_for_check.to_string();

                                tokio::spawn(async move {
                                    // Call the gateway's approve endpoint on localhost.
                                    let url = format!(
                                        "http://127.0.0.1:{port}/api/workflows/runs/{run_id}/approve"
                                    );
                                    ...
                                    match client.post(&url).json(&payload).send().await {
                                        ...
                                        Ok(resp) => {
                                            ...
                                            tracing::warn!(
                                                run_id = %run_id,
                                                %status,
                                                "Discord: workflow approval endpoint returned error"
                                            );
                                        }
                                        Err(e) => {
                                            tracing::warn!(
                                                run_id = %run_id,
                                                error = %e,
                                                "Discord: failed to call workflow approval endpoint"
                                            );
                                        }
                                    }
