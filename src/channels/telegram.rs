...
pub struct TelegramChannel {
    ...
    /// Process-global registry for pending workflow approvals.
...
impl TelegramChannel {
    ...
    /// Attach the process-global approval registry so approval keyword replies
    /// (Telegram messages that reply-to the approval prompt) can be routed to
    /// the gateway's resume endpoint.
    ...
    /// Check whether an incoming Telegram message is a reply to an approval
    /// prompt and, if so, atomically claim the pending approval, POST to the
    /// gateway resume endpoint, and send a confirmation reply. Returns true
    ...
    fn try_intercept_approval(
        ...
        let Some(reply_to) = reply_to_message_id else {
            return false;
        };
        let Some((run_id, is_approve, feedback)) =
            registry.match_telegram_keyword(chat_id, Some(reply_to), clean_text)
        else {
            return false;
        };
                ...
                .await
            {
                ...
                Ok(resp) => {
                    tracing::warn!(
                        run_id = %run_id,
                        status = %resp.status(),
                        "Telegram: workflow approval endpoint returned error"
                    );
                }
                Err(e) => {
                    tracing::warn!(
                        run_id = %run_id,
                        error = %e,
                        "Telegram: failed to call workflow approval endpoint"
                    );
                }
            }
...
Ensure only one `construct` process is using this bot token."
            ...
            if let Some(results) = data.get("result").and_then(serde_json::Value::as_array) {
                for update in results {
                    ...
                    // Human approval keyword intercept — check before typing
