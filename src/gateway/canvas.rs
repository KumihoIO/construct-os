...
pub async fn handle_canvas_post(
    ...
    // Allow localhost (operator) without auth; external callers must authenticate.
...
/// Query parameters for canvas WebSocket.
#[derive(Deserialize)]
pub struct CanvasWsQuery {
...
pub async fn handle_ws_canvas(
    ...
    // Without this, browsers reject the connection when protocols are sent.
    let ws = if headers
        .get("sec-websocket-protocol")
        .and_then(|v| v.to_str().ok())
        .map_or(false, |protos| {
