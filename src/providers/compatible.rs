...
/// OpenAI chat-completions `stream_options` payload. Opting in to
/// `include_usage` instructs the server to emit a final chunk carrying the
/// token usage after the text stream completes.
#[derive(Debug, Serialize)]
...
struct StreamChunkResponse {
    ...
    /// Final usage chunk from servers that honor `stream_options.include_usage`.
    /// Earlier chunks have `choices` populated and `usage` null; the trailing
    /// usage chunk flips this — `choices` is empty and `usage` carries totals.
    #[serde(default)]
    usage: Option<StreamUsageInfo>,
}

#[derive(Debug, Deserialize)]
struct StreamUsageInfo {
    #[serde(default)]
    prompt_tokens: Option<u64>,
    #[serde(default)]
    completion_tokens: Option<u64>,
    #[serde(default)]
    prompt_tokens_details: Option<StreamPromptTokensDetails>,
}

#[derive(Debug, Deserialize)]
struct StreamPromptTokensDetails {
    #[serde(default)]
                ...
                Ok(bytes) => {
                    ...
                    while let Some(pos) = buffer.find('\n') {
                        ...
                        if let Some(usage) = chunk.usage.as_ref() {
                            let token_usage = crate::providers::traits::TokenUsage {
