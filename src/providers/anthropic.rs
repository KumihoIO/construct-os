            ...
            match event_type {
                "message_start" => {
                    let model = event
                        ...
                        .and_then(|m| m.as_str())
                        .unwrap_or("unknown");
                    let usage_obj = event.get("message").and_then(|m| m.get("usage"));
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are Construct"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are Construct".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
