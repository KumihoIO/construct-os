...
#![allow(
    ...
    // See src/lib.rs for the matching block and rationale.
...
mod identity;
mod integrations;
// MCP server runs as an in-process task inside the main daemon. The
// `pub use` re-exports inside `mcp_server/mod.rs` are consumed through the
// library crate (tests), not the bin crate — suppress the resulting
// "unused import" noise here.
...
Examples:
  ...
  construct channel send 'Alert!' --channel-id telegram --recipient 123456789")]
    ...
    /// Manage built-in workflow templates (list, sync to workspace)
    Workflows {
        #[command(subcommand)]
...
from a source checkout; that path will migrate into this subcommand over \
time.
...
Examples:
  ...
  construct install --sidecars-only --skip-kumiho --skip-operator   # no-op smoke test")]
    Install {
        ...
        #[arg(long)]
        sidecars_only: bool,
        /// Skip the Kumiho MCP sidecar install.
        #[arg(long)]
        skip_kumiho: bool,
...
async fn main() -> Result<()> {
    ...
    // Load workspace .env file so secrets (e.g. KUMIHO_SERVICE_TOKEN) are available
    // as environment variables without requiring the user to export them manually.
    let env_path = config.workspace_dir.join(".env");
    if env_path.is_file() {
            ...
            for line in contents.lines() {
                ...
                if let Some((key, value)) = line.split_once('=') {
                    ...
                    if std::env::var(key).is_err() {
                        // SAFETY: called early in main before worker threads are spawned.
    ...
    match cli.command {
        ...
        Commands::Workflows { workflow_command } => match workflow_command {
            WorkflowCommands::List => {
                onboard::builtin_workflows::run_list();
                Ok(())
            }
            WorkflowCommands::Sync { force } => {
                onboard::builtin_workflows::run_sync(config.workspace_dir.clone(), force).await
            }
        },
