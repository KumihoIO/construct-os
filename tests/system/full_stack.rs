...
//! MockProvider → Agent → Tools → Agent response
//!
//! Persistent memory is exclusively provided by Kumiho MCP, so these
//! tests use `NoneMemory` and focus on in-session history/tool flow.
...
/// Multi-turn conversation — verify in-session history accumulation.
#[tokio::test]
...
async fn system_tool_arguments_passed_correctly() {
    ...
    let mut agent = build_agent(provider, vec![Box::new(recording_tool)]);
...
async fn system_parallel_tool_execution() {
    ...
    let mut agent = build_agent(provider, vec![Box::new(EchoTool), Box::new(counting_tool)]);
