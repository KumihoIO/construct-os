...
**`main` is the ONLY default branch. The `master` branch no longer exists.**

If you have an existing fork or local clone that tracks `master`, you **must** update it:

```bash
# Point your local clone at main
git fetch origin
git checkout main                         # create/switch to main tracking origin/main
git branch -D master 2>/dev/null          # delete local master if it still exists
git remote set-head origin main
git fetch origin --prune                  # remove stale remote refs

# If your fork still has a master branch, delete it
...
> **`main`** is the single source-of-truth branch.
>
...
>
> Do **not** create or push to a `master` branch. There is no `master` branch — it will not work.
