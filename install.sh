...
Examples:
  ...
  curl -fsSL https://kumiho.io/install.sh | bash
...
verify_release_artifact() {
  # Verifies a downloaded release archive against SHA256SUMS published in the
  # same GitHub release, and (if cosign is available) verifies the cosign
  # keyless signature on SHA256SUMS. Returns 0 on success, 1 on failure.
  #
  # Arguments:
  ...
  base_url="${archive_url%/${asset_name}}"
  ...
  step_dot "Verifying release artifact integrity..."
  ...
  expected_hash="$(grep -E "[[:space:]]${asset_name}\$" "$sums_path" | awk '{print $1}' | head -n 1)"
  if [[ -z "$expected_hash" ]]; then
    warn "SHA256SUMS does not list ${asset_name}."
    rm -rf "$temp_dir"
    return 1
  fi

  actual_hash="$(compute_sha256 "$archive_path" || true)"
  ...
  if [[ "$expected_hash" != "$actual_hash" ]]; then
    error "SHA256 mismatch for ${asset_name}"
    error "  expected: $expected_hash"
    error "  actual:   $actual_hash"
    rm -rf "$temp_dir"
    return 1
  fi
  step_ok "SHA256 checksum verified"

  if ! have_cmd cosign; then
    step_dot "cosign not installed — skipping signature verification (checksum OK)"
    step_dot "Install cosign (https://docs.sigstore.dev/cosign/installation) for full chain-of-trust verification."
    rm -rf "$temp_dir"
    return 0
  fi

  if ! curl -fsSL "$sig_url" -o "$sig_path" || ! curl -fsSL "$pem_url" -o "$pem_path"; then
    warn "Could not fetch SHA256SUMS.sig / SHA256SUMS.pem — skipping signature verification."
    rm -rf "$temp_dir"
    return 0
  fi
  ...
  if ! COSIGN_EXPERIMENTAL=1 cosign verify-blob \
    ...
    error "cosign signature verification failed for SHA256SUMS"
    error "  identity regexp: $identity_regexp"
    error "  issuer:          $issuer_url"
    rm -rf "$temp_dir"
    return 1
  fi
  step_ok "cosign signature verified (keyless, GitHub OIDC)"
...
install_prebuilt_binary() {
  ...
  if ! verify_release_artifact "$archive_path" "$asset_name" "$archive_url"; then
    warn "Release artifact verification failed. Refusing to install unverified binary."
    rm -rf "$temp_dir"
    return 1
  fi
...
3. Call \`kumiho_memory_engage\` to load relevant context from previous sessions when needed
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --skip-onboard)
      ...
      ;;
    --install-sidecars)
      INSTALL_SIDECARS_MODE="on"
      shift
      ;;
    --skip-sidecars)
      INSTALL_SIDECARS_MODE="off"
      shift
      ;;
    --api-key)
      API_KEY="${2:-}"
...
DESKTOP_DOWNLOAD_URL="https://www.kumiho.io/download"
...
case "$INSTALL_SIDECARS_MODE" in
  on)  _should_install_sidecars=true ;;
  off) _should_install_sidecars=false ;;
  auto)
    if [[ -f "$_sidecar_script" && -d "$WORK_DIR/operator-mcp" ]]; then
      if [[ ! -f "$_kumiho_launcher" || ! -f "$_operator_launcher" ]]; then
        _should_install_sidecars=true
      fi
    fi
    ;;
esac

if [[ "$_should_install_sidecars" == true ]]; then
  if [[ -f "$_sidecar_script" ]]; then
    ...
    if have_cmd python3 || have_cmd python; then
      ...
      warn "python3 not found — skipping sidecar install"
      warn "Install Python 3.11+ and re-run: ./scripts/install-sidecars.sh"
    fi
  else
    warn "scripts/install-sidecars.sh not found — skipping sidecar install"
  fi
elif [[ "$INSTALL_SIDECARS_MODE" == "off" ]]; then
  step_dot "Skipping Python MCP sidecar install (--skip-sidecars)"
fi
