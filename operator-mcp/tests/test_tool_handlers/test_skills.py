...
class TestToolListSkills:
    async def test_returns_skills(self, skills_dir):
        with patch("operator.tool_handlers.skills.list_skills", return_value=[
...
class TestToolLoadSkill:
    async def test_found(self):
        with patch("operator.tool_handlers.skills.load_skill", return_value="# Skill Content"):
            result = await tool_load_skill({"name": "operator-chat"})
            assert result["name"] == "operator-chat"
            assert result["content"] == "# Skill Content"

    async def test_not_found(self):
        with patch("operator.tool_handlers.skills.load_skill", return_value=None):
