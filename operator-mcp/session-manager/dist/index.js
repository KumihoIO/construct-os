...
async function handleRequest(req, res) {
    ...
    try {
        ...
        if (method === "GET" && path[0] === "agents" && path[1] && !path[2]) {
            ...
            if (!info)
                return json(res, 404, { error: `Agent not found: ${path[1]}` });
        ...
        if (method === "GET" && path[0] === "agents" && path[1] && path[2] === "stream") {
            ...
            const unsubscribe = manager.emitter.subscribe(agentId, (event) => {
        ...
        if (method === "GET" && path[0] === "stream" && !path[1]) {
            ...
            const unsubscribe = manager.emitter.subscribeAll((event) => {
                res.write(`data: ${JSON.stringify(event)}\n\n`);
            });
            const heartbeat = setInterval(() => {
                res.write(": heartbeat\n\n");
        ...
        if (method === "GET" && path[0] === "chat" && path[1] === "rooms" && path[2] && !path[3]) {
            ...
            if (!room)
                return json(res, 404, { error: `Room not found: ${path[2]}` });
            return json(res, 200, {
                id: room.id,
                name: room.name,
        ...
        if (method === "POST" && path[0] === "chat" && path[1] === "rooms" && path[2] && path[3] === "messages") {
            const body = JSON.parse(await readBody(req));
            const msg = chat.postMessage(path[2], body.senderId ?? "unknown", body.senderName ?? "unknown", body.content, body.mentions ?? [], body.replyTo);
            if (!msg)
                return json(res, 404, { error: `Room not found: ${path[2]}` });
        ...
        if (method === "POST" && path[0] === "permissions" && path[1] && path[2] === "respond") {
            ...
            if (!success)
                return json(res, 404, { error: `Permission request not found: ${path[1]}` });
...
function main() {
    ...
    const socketPath = socketArg >= 0 && process.argv[socketArg + 1]
        ...
        : `${process.env.HOME}/.construct/operator/session-manager.sock`;
