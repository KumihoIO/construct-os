...
export class WebSocketClient {
  ...
  connect(): void {
    ...
    // Close any existing socket before creating a new one.  If the old socket
    // is orphaned (e.g. a reconnect fires while the previous one is still
    // open), its onmessage handler still holds a reference to this.onMessage
    // and would deliver duplicate events.  Nulling the handlers first
    // prevents that, and explicitly closing ensures the server-side session
    // moves cleanly to the new connection.
    if (this.ws) {
      this.ws.onopen = null;
    ...
    const sessionId = this.sessionId ?? getOrCreateSessionId();
